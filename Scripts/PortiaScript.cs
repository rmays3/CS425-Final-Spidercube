...
public class PortiaScript : MonoBehaviour {
        ...
        if (rotationStarted)
        {
            if(rCount < transitionFrames){
                ...
                if (rCount == transitionFrames - 1)
                {
                    ...
                    //Debug.Log(pTOriginal.z + ", r = " + rCount);
                    Debug.Log(rTOriginal + ", " + characterController.transform.eulerAngles);
                    //Debug.Log(characterController.transform.rotation);
                }
                else
                {
        ...
        else if (start && !rotationStarted)
        {
            ...
            else if(noTransition)
            {
                ...
                if (Input.GetAxis("Vertical") > 0.00F && Input.GetAxis("Horizontal") == 0.00F)
                {
                    ...
                    //characterController.transform.rotation = camTransform.rotation;
                    ...
                    {
                        characterController.transform.eulerAngles = camAngles;
                        relativeDir = camAngles;
                    }
                    cardDir = 1;
                    rotLock = true;
                }
        ...
        if(sum > 0)
        {
            ...
            else if (xFace == 1 && zFace == -1)
            {
                ...
                if (wdir.x > 0.0F)
                {
                    ...
                    placeTransition.x = (platTransform.position.x + (platTransform.localScale.x / 2.0F)) + (characterController.transform.localScale.x / 2.0F);
                    placeTransition.z = platTransform.position.z - (platTransform.localScale.z / 2.0F);
                    placeTransition.y = characterController.transform.position.z;
            ...
            else if (yFace == 1 && zFace == 1)
            {
                ...
                else if (wdir.z > 0.0F)
                {
                    ...
                    placeTransition.y = platTransform.position.y + (platTransform.localScale.y / 2.0F);
                    placeTransition.x = characterController.transform.position.x;
                    //Debug.Log(platTransform.position.z + (platTransform.localScale.z / 2.0F));
                    //Debug.Log(placeTransition.z);
            ...
            else if (yFace == 1 && zFace == -1)
            {
                ...
                {
                    //Debug.Log("Aw yeah!" + (wdir.z < 0.0F) + ", " + wdir.z);
                    v2.z = v1.y;
                    v2.x += 90.0F;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "Platform" && platName != other.gameObject.name)
        {
            if(start)
            {
                //distTen = (other.gameObject.transform.position - platTransform.position) / 35.0F;
                //i = 0;
                //fairyIsMoving = true;
            }
            ...
            //start = true;
            isColliding = true;
            //Debug.Log("We collided with the platform.");
    ...
    private void LateUpdate()
    {
        ...
        if (Input.GetAxis("CamVertical") != 0.00F)
        {
        ...
        if (Input.GetAxis("CamHorizontal") != 0.00F)
        {
